...
async function main(cookiesArr) {
    await getList();
    console.log(`共有${$.activityId.length}个活动，分别是：${$.activityId}`)
    //await openActivity()
    for (let i = 0; i < $.activityId.length; i++) {
        await helpFirst($.activityId[i], cookiesArr)
            ...
            try {
                if (res) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].status == "ON_GOING") {
                            $.activityId.push(data[i].activeId)
                        }
                    }
...
async function helpFirst(activityId, cookiesArr) {
    for (let i = 0; i < cookiesArr.length; i++) {
        let cookieTemp = cookiesArr[i];
        new Promise((resolve) => {
            let options = {
                "url": `https://draw.jdfcloud.com/common/api/bean/activity/participate?activityId=${activityId}&inviteUserPin=${$.UserName}&invokeKey=NRp8OPxZMFXmGkaE`,
                "headers": {
